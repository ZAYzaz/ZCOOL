...
var ActivityModel = mongoose.model('activity', new mongoose.Schema({
...
const listall = (_query = {}) => {
    ...
    return ActivityModel.find(_query).sort({ createTime: -1 }).then((results) => {
        return results
    }).catch((err) => {
        return false
    })
...
const list = async ({ pageNo = 1, pageSize = 10, search = '' }) => {
    ...
    return ActivityModel.find(_query)
        .sort({ createTime: -1 })
...
const remove = async ({ id, pageNo, pageSize }) => {
    ...
    return ActivityModel.deleteOne({ _id: id }).then(async (results) => {
...
const listone = ({ id }) => {
    //console.log(ActivityModel.findById(id));

    return ActivityModel.findById({ _id: id }).then((results) => {
...
const update = (body) => {
    ...
    return ActivityModel.updateOne({ _id: body.id }, { ...body }).then((results) => {
        return results
    }).catch((err) => {
        return false
    })
